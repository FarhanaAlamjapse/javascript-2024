...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    // console.log(locale); //locale lang will run if we dont set

    labelDate.textContent = new Intl.DateTimeFormat(
      currentAccount.locale,
      options
    ).format(now); //this will create so called formatter for this language
...
console.log('waiting....');
if (ingredients.includes('olives')) clearTimeout(pizzaTimer); //will not printed
//setinterval
setInterval(function () {
  const now = new Date();
  console.log(now);
}, 1000); //it will print every 1 sec,1000 is millisecond
/*
//intl numbers:
